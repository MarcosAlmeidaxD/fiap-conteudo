...
# Fix Unicode output on Windows (cp1252 doesn't support emojis)
...
def process_folder(folder: Path):
    ...
    for i, file_path in enumerate(files, 1):
        ...
        if out_path.exists():
            ...
            if "[Gemini error:" not in existing:
...
def main():
    ...
    parser.add_argument("--output-dir", help="Custom output directory (overrides date-based default)")
    ...
    folder = Path(args.folder) if args.folder else BASE_DIR / "demo_files"
